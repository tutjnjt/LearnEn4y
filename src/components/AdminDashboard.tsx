...
import { Users, Star, Trophy, Clock, LogOut, ChevronRight, Play } from "lucide-react";
import { useAuth } from "../contexts/AuthContext";
import { AdminUserDetail } from "./AdminUserDetail";

export function AdminDashboard({ onPlay }: { onPlay?: () => void }) {
          ...
          <>
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <div className="text-slate-500 font-medium mt-2">Quản lý người chơi Eng4Bun</div>
              </div>
              <div className="flex gap-2">
                {onPlay && (
                  <button 
                    onClick={onPlay}
                    className="flex items-center gap-2 px-5 py-3 bg-purple-600 border-2 border-purple-700 text-white rounded-xl hover:bg-purple-700 transition-colors font-bold shadow-sm"
                  >
                    <Play className="w-5 h-5" /> Chơi Game
                  </button>
                )}
                <button 
                  onClick={logout}
                        ...
                        <tr key={i} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-right">
                            <button
                              ...
                              className="inline-flex items-center justify-center gap-1 px-4 py-2 bg-slate-100 text-slate-700 font-bold rounded-xl hover:bg-slate-200 transition-colors border-2 border-slate-200"
                            >
